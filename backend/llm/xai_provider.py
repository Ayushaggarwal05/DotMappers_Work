...
from backend.query_engine.intent_schema import (
    ...
    FilterOperator, FilterClause, SortSpec, SortOrder,
    DateRangeSpec, RelativePeriod
)


class RuleBasedFallbackProvider(BaseLLMProvider):
    """
    Zero-Cost Deterministic Semantic Intent Parser.
    
    Used when XAI_API_KEY is not configured or in offline/evaluation mode.
    Guarantees 100% zero-cost execution for all assessment queries.
    """
...
class XAIProvider(BaseLLMProvider):
    """
    xAI / Grok LLM Provider for natural language intent translation.
    
    Communicates with xAI Chat Completion API.
    If no API key is configured, transparently falls back to RuleBasedFallbackProvider.
    """
    ...
    async def generate_intent(self, question: str) -> StructuredIntent:
        # If no API key is provided, use deterministic zero-cost fallback
        if not self.api_key or not self.api_key.strip():
            logger.info("No XAI_API_KEY configured; utilizing deterministic semantic fallback parser.")
            return await self.fallback.generate_intent(question)

        logger.info(f"Calling xAI API ({self.model}) for query translation: '{question}'")
        ...
        # Attempt call with 1-shot retry on parsing failure
        for attempt in range(2):
            try:
                ...
                logger.warning(f"Parse error on attempt {attempt + 1}: {pe}. Retrying...")
                if attempt == 1:
                    logger.warning("LLM response failed validation after retry. Falling back to rule parser.")
                    return await self.fallback.generate_intent(question)
